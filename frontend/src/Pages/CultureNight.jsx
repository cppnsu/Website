...
const CultureNight = () => {
  ...
  if (data) {
    ...
    const { CN_Date, Description, English_definition, English_romaji, Featured, Gallery, Japanese_title, Price, Summary, Time_start, more_info_pdf } = data.data.getCultureNight[0]
    ...
    const cnNumberString = (cnNumber) => {
      ...
      switch (ending) {
        case (1): return `${cnNumber}st`
        case (2): return `${cnNumber}nd`
        case (3): return `${cnNumber}rd`
        default: return `${cnNumber}th`
      }
    }
    ...
    const formattedDate = () => {
      return String(format(Date.parse(CN_Date), 'MMMM dd, Y', { timeZone: 'America/Los_Angeles' }))
    }
    ...
    return (
      <div className="w-screen h-full py-32 bg-zinc-800">
        ...
        <div className="w-5/6 max-w-3xl m-auto">
          <div>
            <h1 className="text-2xl mt-4 text-center">Join us for CPP NSU&apos;s {numCN()} annual Culture Night on {formattedDate()}!</h1>
            <h1 className="text-2xl mt-2 text-center">More info:</h1>
            {numPages > 1 && <p className="mt-4"> Page {currentPageNumber} of {numPages} </p>}
            {numPages >= 1 ?
              <Document file={more_info_pdf} onLoadSuccess={onDocumentLoadSuccess} onLoadError={ErrorLoadingPDF}  >
                <Page renderTextLayer={false} pageNumber={currentPageNumber} />
              </Document>
              :
              <div className="w-full h-full" >
                <p className="text-lg text-center">We couldn&apos;t load the file, please try refreshing the page</p>
              </div>
            }
            {numPages > 1 &&
