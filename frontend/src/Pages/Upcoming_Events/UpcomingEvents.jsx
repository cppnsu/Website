  ...
  if (data) {
    ...
    return (
      <div className=" w-screen h-max bg-zinc-800 py-32">
        <h1 className="text-slate-100 text-6xl text-center pb-8">
          UPCOMING EVENTS
        </h1>
        {events.map((event) => {
          return (
            <div
              key={`${event.Name}--${event.Time_Start}`}
              className="h-1/3 w-4/5 mx-auto py-3"
            >
              ...
              <div className="flex flex-row justify-center align-middle">
                <div className=" mt-6 shadow bg-rose-700 w-32 h-32 flex flex-col justify-center ">
                  <h1 className="drop-shadow-lg text-slate-100 text-3xl text-center">
                    {format(Date.parse(event.Date_Start), "MMM", {
                      timeZone: "America/Los_Angeles",
                    })}
                  </h1>
                  <h1 className="drop-shadow-lg text-slate-100 text-5xl text-center ">
                    {format(Date.parse(event.Date_Start), "d", {
                      timeZone: "America/Los_Angeles",
                    })}
                  </h1>
                </div>
                <div className="w-4/5 h-full flex flex-col space-y-3 ml-10 ">
                  ...
                  <button
                    ...
                    className="text-slate-100 bg-rose-700 w-28 h-8 rounded-full text-xl"
                  >
